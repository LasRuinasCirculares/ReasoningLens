			...
			<div class=" mb-1.5" slot="content">
				...
				{#if token?.attributes?.type === 'reasoning'}
					<div class="flex justify-end pt-1">
						<button
							class="px-3 py-1 text-xs rounded-full border border-gray-200 dark:border-gray-700 text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-800 transition"
							on:click|stopPropagation={() =>
								dispatch('analyzeReasoning', {
									reasoning: decode(token.text)?.trim?.() ?? '',
									attributes: token.attributes ?? {}
								})}
						>
							{$i18n.t('Analyze')}
