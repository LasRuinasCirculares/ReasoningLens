...
# Note: StreamingReasoningSectionBuilder and split_reasoning_into_sections
# are imported lazily to avoid circular imports
# See extract_reasoning_sections_from_blocks function and response_handler
...
def _finalize_reasoning_sections(
    reasoning_section_builder, content_blocks: list
) -> Optional[list]:
    """
    Finalize reasoning sections from streaming builder or content blocks.

    This helper function consolidates the logic for extracting reasoning sections
    that was previously duplicated in multiple places.

    Args:
        reasoning_section_builder: StreamingReasoningSectionBuilder instance or None
        content_blocks: List of content blocks from the response

    Returns:
        List of section dictionaries or None if no reasoning content
    """
    reasoning_sections = None

    if reasoning_section_builder is not None:
        # Check if builder has content (from reasoning_content field)
        if reasoning_section_builder.total_content:
            reasoning_sections = reasoning_section_builder.finalize()
        else:
            # Builder is empty, meaning reasoning came from tag detection
            # Feed all reasoning content from content_blocks to builder
            for block in content_blocks:
                if block.get("type") == "reasoning":
                    block_content = block.get("content", "")
                    if block_content:
                        reasoning_section_builder.append(block_content)
                        # Add separator between blocks
                        reasoning_section_builder.append("\n\n")
            reasoning_sections = reasoning_section_builder.finalize()

    # Fall back to original extraction if builder failed or returned empty
    if not reasoning_sections:
        reasoning_sections = extract_reasoning_sections_from_blocks(content_blocks)

    return reasoning_sections


def extract_reasoning_sections_from_blocks(content_blocks: list) -> Optional[list]:
    """
    Extract reasoning content from content_blocks and split into sections.

    This function is called when a response is complete to pre-compute
    reasoning sections for faster analysis later.
    ...
    # Collect all reasoning content from blocks
    reasoning_texts = []
    for block in content_blocks:
        if block.get("type") == "reasoning":
    ...
    # Join all reasoning texts
    full_reasoning = "\n\n".join(reasoning_texts)

    # Split into sections using the smart splitting function
    # Import here to avoid circular imports
    try:
        from open_webui.routers.reasoning_analysis.extractors import (
            split_reasoning_into_sections,
        )

        sections = split_reasoning_into_sections(full_reasoning)
        return sections
    except Exception as e:
        log.warning(f"Failed to split reasoning into sections: {e}")
        return None
...
async def process_chat_response(
    ...
    if event_emitter and event_caller:
        ...
        async def response_handler(response, events):
            ...
            # Create streaming section builder for incremental reasoning section splitting
            ...
            try:
                ...
                reasoning_section_builder = StreamingReasoningSectionBuilder()
            except Exception as e:
                log.warning(f"Failed to create StreamingReasoningSectionBuilder: {e}")
                reasoning_section_builder = None
            ...
            try:
                            ...
                            if data:
                                ...
                                if "selected_model_id" in data:
                                    ...
                                    if reasoning_content:
                                        ...
                                        # Append to streaming section builder for incremental section splitting
                                        if reasoning_section_builder is not None:
                                            reasoning_section_builder.append(
                                                reasoning_content
                                            )
                ...
                # Get reasoning sections from streaming builder or content blocks
                reasoning_sections = _finalize_reasoning_sections(
                    reasoning_section_builder, content_blocks
                )
                ...
                if not ENABLE_REALTIME_CHAT_SAVE:
                    ...
                    if reasoning_sections:
                        message_data["reasoning_sections"] = reasoning_sections
                    ...
                    # In realtime save mode, update with reasoning_sections and content_blocks
                    update_data = {
                        "content_blocks": content_blocks,
                    }
                    if reasoning_sections:
                        update_data["reasoning_sections"] = reasoning_sections
