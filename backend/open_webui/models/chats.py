    ...
    def update_chat_by_id(self, id: str, chat: dict) -> Optional[ChatModel]:
        try:
            with get_db() as db:
                ...
                # Preserve content_blocks and reasoning_sections from existing messages
                # when updating chat to prevent frontend from overwriting backend-saved data
                ...
                new_history = chat.get("history", {})
                new_messages = new_history.get("messages", {})

                # Merge messages, preserving content_blocks and reasoning_sections
                preserved_fields = ["content_blocks", "reasoning_sections"]
                for msg_id, existing_msg in existing_messages.items():
                    if msg_id in new_messages:
                        for field in preserved_fields:
                            if (
                                field in existing_msg
                                and field not in new_messages[msg_id]
                            ):
                                new_messages[msg_id][field] = existing_msg[field]

                if new_messages:
                    chat["history"]["messages"] = new_messages
